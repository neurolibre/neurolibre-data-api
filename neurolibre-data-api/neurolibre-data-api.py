...
def zenodo_create_bucket(title, archive_type, creators, user_url, fork_url, commit_user, commit_fork, issue_id):
    ...
    if (archive_type == 'book'):
        ...
        data["metadata"]["upload_type"] = 'software'
        data["metadata"]["description"] = 'Docker image built from the ' + libre_text + ', based on the ' + user_text + f", using repo2docker (through BinderHub). <br> To run locally: <ol> <li><strong><code>docker load < DockerImage_10.55458_NeuroLibre_{'%05d'%issue_id}_{commit_fork[0:5]}.zip</code></strong></li><li><strong><code>docker run -it --rm -p 8888:8888 DOCKER_IMAGE_ID jupyter lab --ip 0.0.0.0</code></strong><br> by replacing <code>DOCKER_IMAGE_ID</code> with the respective ID of the Docker image loaded from the zip file.</li></ol>" + review_text + sign_text
